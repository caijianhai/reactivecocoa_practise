...
- (void)viewDidLoad {
    ...
    RACSignal *stepTimeSignal = [[RACSignal switch:[clickSignal replayLast]
                                  cases:@{@(ControlStateStop): [RACSignal empty]
                                          }
                                  default:timerSignal] take:steps.count];
    
    RACSignal *stepsSignal = stepsSequence.signal;
    
    stepsSignal = [[[[stepsSignal concat:[RACSignal never]] sample:stepTimeSignal]
                    scanWithStart:RACTuplePack(nil, @0) reduce:^id(RACTuple* running, RACTuple* next) {
                        
                        NSNumber *idx = running.second;
                        NSInteger nextIdx = (idx.integerValue + 1) % steps.count;
                        return RACTuplePack(stepsSequence.array[nextIdx], @(nextIdx));
                    }] reduceEach:^id(RACTuple *steps, id _){
